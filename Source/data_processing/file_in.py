...
def check_col_names(path, pvalue=False):
    ...
    Args:
        ...
        pvalue: if True, check for a pvalue in df from path
    ...
    if pval_match is None:
        pval = re.compile(r'.*p.*val.*', re.I)
        for i in cols:
            if pval.search(i) is not None:
                pval_match = re.search(pval, i).group()
    if pvalue:
        ...
        return {'symbol': gene_match, 'logfc': log_match}
